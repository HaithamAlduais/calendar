...
// ── ٣. تغيير مدة الصلاة يُزيح الإشعار ────────────────────────────
{
  ...
  // الروتين يلي الفجر مباشرة، فتقصيرُ الفجر يقدّمه
  const after = (s) => blocksAround(s, NOW).filter((x) => x.title === 'الروتين').map((x) => x.minute);
  const a = after(riyadh);
  const c = after(shifted);
  ok('فجرٌ أقصر ← الروتين يبدأ أبكر', c.some((m, i) => m < a[i]), `${a[0]} → ${c[0]}`);
}
...
// ── ٤. من دار يومه على المغرب اختلف ترتيب بلوكاته ────────────────
{
  ...
  ok('ويوم الرياض يفتتحه الراحة (الفَحْمة)', firstR.title === 'راحة', `= ${firstR.title}`);
}
