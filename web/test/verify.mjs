...
// ── توزيع أنصاف التثبيت على السنن: نصفٌ واحد لكل سنّة، بترتيب موعد قراءتها ──
// كانت القاعدةُ «i ناقص عددُ الفائتات قبلها»، فإذا فاتت سنّتان تاليتان حملتا
// النصفَ نفسَه، وإذا فات اليومُ كلُّه حملت السننُ الثماني نصفًا واحدًا — فيُكتب
// في القضاء موضعٌ واحد ثماني مرات. والقاعدةُ الآن ترتيبُ المواعيد، وهي حقنٌ
// (bijection) لا يتكرّر فيها نصف.
{
  const { wirdOrder } = await import('../lib/engine/schedule.js');
  const T = (h) => `2026-08-29T${String(h).padStart(2, '0')}:00`;
  // مواعيد السنن الثماني في يومٍ نموذجي
  const at = [T(4), T(6), T(12), T(12), T(15), T(18), T(19), T(20)];

  // (أ) يومٌ لم يفت منه شيء: كلُّ سنّةٍ نصفُها بترتيبها
  check('لا فوات: الترتيب طبيعي', wirdOrder(at.map((a) => ({ at: a }))).join(','), '0,1,2,3,4,5,6,7');

  // (ب) مثالُه بنصّه: فاتت سنّتا الفجر والضحى وهو في الظهر، وقضاؤهما ١٢:٣٩
  const case2 = at.map((a, i) => (i < 2 ? { at: a, makeupAt: '2026-08-29T12:39' } : { at: a }));
  const o2 = wirdOrder(case2);
  check('سنّتا الظهر تأخذان النصفين الأول والثاني', [o2[2], o2[3]].join(','), '0,1');
  check('وقضاءُ الفجر والضحى الثالثَ والرابع', [o2[0], o2[1]].join(','), '2,3');
  check('ثم العصرُ فما بعده على ما بقي', [o2[4], o2[5], o2[6], o2[7]].join(','), '4,5,6,7');

  // (ج) اليومُ كلُّه فات: ثمانيةُ أنصافٍ متمايزة لا نصفٌ واحد ثماني مرات
  const o3 = wirdOrder(at.map((a) => ({ at: a, makeupAt: '2026-08-29T21:00' })));
  check('فوات اليوم كلِّه: ثمانية أنصاف متمايزة', new Set(o3).size, 8);

  // (د) القاعدة حقنٌ في كل الصور: لا نصفَ يتكرّر ولا نصفَ يسقط
  for (let mask = 0; mask < 256; mask++) {
    const o = wirdOrder(at.map((a, i) => (mask & (1 << i) ? { at: a, makeupAt: T(13) } : { at: a })));
    if (new Set(o).size !== 8 || o.some((x) => x < 0 || x > 7)) {
      check(`تكرار عند القناع ${mask}`, o.join(','), 'بلا تكرار');
      break;
    }
  }
  ok('لا تكرار في ٢٥٦ صورة من صور الفوات', true);
}

// ── بلوكُ مهام اليوم العائمة لا يفوت ولا تُقضى بنودُه ──────────────────────
// مهمتا القرآن والتمرين متاحتان في كل بلوك مهام طوال اليوم، فترحيلُهما قضاءً
// يحسبهما مرتين — مرةً عائمتين ومرةً مقضيّتين.
{
  const routine = HC.templates.day.blocks.find((b) => b.id === 'routine');
  ok('الروتين مضيفُ المهام العائمة', routine.floats === true && routine.task === true);
}
